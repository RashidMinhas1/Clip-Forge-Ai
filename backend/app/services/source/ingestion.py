...
class SourceIngestionService:
    ...
    async def ingest_local_file(self, project_id: uuid.UUID, filename: str, file_stream: BinaryIO, size_bytes: int) -> SourceMetadata:
        """
        Ingests a local file asynchronously and persists to project.
        """
        ...
        try:
            ...
            metadata = self._build_error_response(source_id, "local", e.error_code, str(e))
        except MediaProbeException as e:
            logger.error(f"Media validation error for {filename}: {str(e)}")
            metadata = self._build_error_response(source_id, "local", "MEDIA_VALIDATION_FAILED", str(e))
        except Exception as e:
            logger.error(f"Unexpected error during local ingestion: {str(e)}")
            metadata = self._build_error_response(source_id, "local", "INTERNAL_ERROR", "An unexpected error occurred")

        if self.source_repository:
            await self.source_repository.create(project_id, metadata)
        
        return metadata

    async def ingest_youtube_url(self, project_id: uuid.UUID, url: str) -> SourceMetadata:
        """
        Ingests a YouTube video asynchronously and persists to project.
        """
        source_id = str(uuid.uuid4())
        normalized_url = url
        metadata = None
        ...
        try:
            ...
            metadata = self._build_error_response(source_id, "youtube", e.error_code, str(e), original_url=url, normalized_url=normalized_url)
            ...
            metadata = self._build_error_response(source_id, "youtube", "MEDIA_VALIDATION_FAILED", str(e), original_url=url, normalized_url=normalized_url)
        except Exception as e:
            logger.error(f"Unexpected error during YouTube ingestion: {str(e)}")
            metadata = self._build_error_response(source_id, "youtube", "INTERNAL_ERROR", "An unexpected error occurred", original_url=url, normalized_url=normalized_url)
